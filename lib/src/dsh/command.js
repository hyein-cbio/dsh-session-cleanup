...
import { buildDshSessionLabel, listDshSessions, } from "./list-sessions.js";
...
function currentSessionId(invocation) {
    const id = invocation.agent?.session?.id;
    return typeof id === "string" && id.length > 0 ? id : undefined;
}
function currentCwd(invocation) {
...
async function confirmDelete(questions, invocation, sessions) {
    if (!questions) {
        return true;
    }
...
async function pickSessions(questions, invocation, sessions) {
    ...
    const selected = new Set(answer.answers.find((item) => item.id === "sessions")?.selected ?? []);
    if (selected.size === 0) {
        return "cancelled";
    }
    return sessions.filter((session, index) => selected.has(buildDshSessionLabel(session, index)));
}
function formatDeleteOutcome(session, outcome) {
    const parts = [
...
export async function handleDshSessionCleanupCommand(ctx, invocation) {
    ...
    try {
        ...
        if (parsed.kind === "delete") {
            selected = listed.filter((session) => parsed.ids.some((id) => idsMatch(session.id, id)));
            const missing = parsed.ids.filter((id) => !selected.some((session) => idsMatch(session.id, id)));
            if (missing.length > 0 && selected.length === 0) {
                return result("error", `No matching sessions: ${missing.join(", ")}`);
            }
        }
        else if (listed.length === 0) {
            const empty = parsed.scope === "orphaned"
        ...
        return result("success", `Deleted ${deleted.length} session(s), but ${failed.length} failed.\n${deleted.join("\n")}\n${failed.join("\n")}`);
    }
    catch (error) {
