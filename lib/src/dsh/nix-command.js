...
import { commandResult, currentSessionId, } from "./invocation.js";
import { dshNixUsage, parseDshNixArgs } from "./nix-parse.js";
import { switchToNewSession } from "./tui-switch.js";
function notifyHost(ctx, text, color) {
    const toast = optionalService(ctx, "tuiToast");
...
export async function handleDshNixCommand(ctx, invocation, hooks = {}) {
    ...
    try {
        ...
        const confirmed = await confirmAction(questions, invocation, "Delete this session and start a new one?", [
        ...
        if (deleteError) {
            notifyHost(ctx, `New session started, but the previous session could not be deleted: ${deleteError}`, "warning");
            return commandResult("success", `New session started. The previous session could not be deleted: ${deleteError}`);
        }
